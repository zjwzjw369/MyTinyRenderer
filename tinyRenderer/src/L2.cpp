...
void triangle_barycentric(vec2 *pts, TGAImage &image, TGAColor color) {
...
void drawTriangle(Model* model, TGAImage &image) {
	...
	for (int i = 0; i < model->nfaces(); i++) {
		...
		// 背面剔除
		if (intensity > 0) {
			vec2 pts[3] = { vec2(x0,y0), vec2(x1, y1), vec2(x2, y2) };
			triangle_barycentric(pts, image, TGAColor(rand() * 255, rand() * 255, rand() * 255, 255));
		}
	}
}
