...
void drawTriangle(Model* model, TGAImage &image, mat4 vpv_mat) {
	...
	for (int i = 0; i < model->nfaces(); i++) {
		...
		vec3 v0_screen_space = proj<3>(vpv_mat * embed<4>(v0, 1.0));
		vec3 v1_screen_space = proj<3>(vpv_mat * embed<4>(v1, 1.0));
		vec3 v2_screen_space = proj<3>(vpv_mat * embed<4>(v2, 1.0));

		vec3 n = cross((v2_screen_space - v0_screen_space), (v1_screen_space - v0_screen_space));
		n.normalize();
		float intensity = n * light_dir;
		int col = int(intensity * 255);
		// 背面剔除
		if (intensity > 0) {
			vec3 pts[3] = {v0_screen_space, v1_screen_space, v2_screen_space};
...
int main(int argc, char** argv) {
	...
	drawTriangle(&model, image, vpv_mat);
